...
void ExpressionTree::create_final(Node* node) {
	if (check_formula(node) == 1) {
		...
		string mult = node->left->get_value(), plus = node->right->left->get_value();
		node->set_value(node->right->get_value());
		node->right->set_value("*");
		node->left->set_value("*");
		node->right->left->set_value(mult);
		node->left->right = new Node;
		node->left->right->set_value(plus);
		node->left->left = new Node;
		node->left->left->set_value(mult);
	}
}
...
void ExpressionTree::print_expression(Node* node) {
	if (node) {
		if (node->get_value() == "*" or node->get_value() == "/" or node->get_value() == "+" or node->get_value() == "-") {
			cout << "(";
		}
